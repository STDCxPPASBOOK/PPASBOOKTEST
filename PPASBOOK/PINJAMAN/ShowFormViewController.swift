...
//  PPASBOOK
//
//  Created by STDC_20 on 09/08/2024.
//

import UIKit

class ShowFormViewController: UIViewController, UITableViewDataSource, UITableViewDelegate {
    ...
    @IBAction func unwindToShowFormViewController(segue: UIStoryboardSegue) {
        if segue.source is DetailLaptopViewController {
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "showDetailSegue" {
            if let detailVC = segue.destination as? DetailLaptopViewController,
               let selectedItem = sender as? CustomFormItem {
                detailVC.item = selectedItem // Pass the selected item to the detail view controller
            }
        }
    }
